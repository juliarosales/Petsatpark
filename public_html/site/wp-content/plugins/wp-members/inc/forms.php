...
function wpmem_login_form( $page, $arr ) {
	...
	$defaults = array(
		...
		'main_div_before' => '<h4>Existing members Log in</h4><div id="wpmem_login">',
...
function wpmem_inc_registration( $toggle = 'new', $heading = '', $redirect_to = null ) {
	...
	$defaults = array(
		...
		'main_div_before'  => '<h4>New member registration</h4><div id="wpmem_reg">',
	...
	foreach ( $wpmem_fields as $field ) {
		...
		if ( $field[4] == 'y' && $do_row == true ) {
			...
			if ( $field[2] == 'tos' ) {
				...
				$input.= apply_filters( 'wpmem_tos_link_txt', sprintf( __( 'Please indicate that you agree to the %s Terms and Conditions %s', 'wp-members' ), $tos_pop, '</a>' ), $toggle );
	...
	if ( ( $wpmem->captcha == 1 || $wpmem->captcha == 3 ) && $toggle != 'edit' ) { // don't show on edit page!
		...
		$row = '<div class="frame"><iframe src="http://www.petsatpark.com/site/wp-content/plugins/wp-members/wp-members-tos.php" height="150" width="100%" frameborder="0"></iframe></div>';
